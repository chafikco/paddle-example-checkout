...
const navigation = [
  ...
  { name: 'Edit Payment Method', href: '/edit', icon: FolderIcon, current: false },
...
export default function Dashboard({ customers }) {
  ...
  const [subId, setSubId] = useState('sub_id');
  ...
  useEffect(() => {
    ...
    const getApiData = async (transaction_id) => {
      const response = await fetch("http://localhost:3000/api/getTransaction", { method: "POST", body: transaction_id });
      const data = await response.json();
      setSubId(data.manageData.subscription_id)
      //and another session storage for sub_id
      sessionStorage.setItem("subscription_id", data.manageData.subscription_id);
    }
    getApiData(transaction_id);
  }, []);
          ...
          <main className="flex-1">
            <div className="py-6">
              ...
              <div className="mx-auto max-w-7xl px-4 sm:px-6 md:px-8">
                ...
                <div className="py-4">
                   <PlanStats subscription_id={subId}/>
                </div>
                {/* /End replace */}
              </div>
            </div>
